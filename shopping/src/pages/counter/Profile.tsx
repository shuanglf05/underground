...
import { 
  ...
  LogOut, Bell, Shield, FileText, ChevronRight, ShoppingCart, Ticket
} from 'lucide-react';

export default function CounterProfile() {
  const navigate = useNavigate();

  const menuItems = [
    { icon: Bell, label: '消息通知', path: '/counter/notifications' },
    { icon: Shield, label: '账号安全', path: '/counter/security' },
    { icon: FileText, label: '操作日志', path: '/counter/logs' },
    { icon: HelpCircle, label: '帮助中心', path: '/counter/help' },
    { icon: Settings, label: '系统设置', path: '/counter/settings' },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="bg-white mx-4 -mt-6 rounded-xl p-4 shadow-lg relative z-10">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h2 className="text-lg font-bold text-gray-900">李吧台</h2>
            <p className="text-sm text-gray-500">吧台操作员</p>
            <div className="flex items-center gap-2 mt-2">
              <span className="px-2 py-0.5 bg-purple-100 text-purple-600 text-xs rounded-full">在线</span>
              <span className="text-xs text-gray-500">当前班次: 08:00-16:00</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white mx-4 mt-2 rounded-xl p-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center">
            <p className="text-2xl font-bold text-purple-600">¥15,680</p>
            <p className="text-xs text-gray-500 mt-1">销售额</p>
          </div>
          <div className="text-center">
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-gray-500 mt-1">手环发放</p>
          </div>
        </div>
      </div>
      ...
      <button 
        ...
        className="w-full mx-4 mt-4 py-3 bg-white rounded-xl text-red-600 font-medium hover:bg-red-50 transition-colors"
      >
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-2">
        <div className="flex items-center justify-around">
          ...
          <button onClick={() => navigate('/counter/profile')} className="flex flex-col items-center gap-1 text-purple-600">
